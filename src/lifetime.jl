function select_lifetime_global_polynomial(X_data::Vector{NTuple{d,Float64}},
    ...
    n = length(X_data)
    derivative_estimates = get_derivative_estimates_global_polynomial(X_data, Y_data, debias_order)
    sigma2_hat = get_variance_estimate_global_polynomial(X_data, Y_data, debias_order)

    numerator = (4 * debias_order + 4) / (debias_order + 2)^2
...
function get_derivative_estimates_global_polynomial(X_data::Vector{NTuple{d,Float64}},
                                                    Y_data::Vector{Float64},
                                                    debias_order::Int) where {d}
    n = length(X_data)
    ...
    for i in 1:n
        for j in 1:d
            derivative_estimate = factorial(2 * debias_order + 2) *
                                  derivative_vectors[i][j]' * regression_vector
...
function get_variance_estimate_global_polynomial(X_data::Vector{NTuple{d,Float64}},
                                                 Y_data::Vector{Float64},
                                                 debias_order::Int) where {d}
    n = length(X_data)
    design_matrix = make_design_matrix_global_polynomial(X_data, debias_order)
    ...
    return sigma2_hat /= n - (2 * debias_order + 4) * d - 1
end
