"""
    select_lifetime_polynomial_amse(X_data::Vector{NTuple{d,Float64}}, Y_data::Vector{Float64},
                                    x_eval::NTuple{d,Float64}, debias_order::Int=0) where {d}

Select the lifetime parameter for a (debiased) Mondrian random forest
using polynomial estimation by minimizing the
...
Y_data= data["Y"]
debias_order = 0
lambda = select_lifetime_polynomial_amse(X_data, Y_data, debias_order)
```
"""
