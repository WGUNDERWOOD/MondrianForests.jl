using PyPlot

# start plot
plt.ioff()
(fig, ax) = plt.subplots(figsize=(5, 5))
for side in ["bottom", "top", "left", "right"]
    ax.spines[side].set_color("#FFFFFF00")
end
plt.xticks([])
plt.yticks([])

# curve and box
lw = 12
n = 200
xs = range(0, 1, length=n)
ys = (1.2 .* xs .- 0.63).^3 .+ 0.60 - 0.2 * xs.^2 -
0.05 * (0.5 .* xs .+ 0.5).^10 +
0.07 * (1 .- xs).^10
xs = [xs; [1, 0, xs[1]]]
ys = [ys; [0, 0, ys[1]]]
plt.plot(xs, ys, c="#111111", lw=lw)

# piet lines
x1 = 0.25
x2 = 0.80
y1 = 0.11
y2 = 0.25
plt.plot([x1, x1], [0, 0.55], c="#111111", lw=lw)
plt.plot([0, 1], [y2, y2], c="#111111", lw=lw)
plt.plot([x2, x2], [0, y2], c="#111111", lw=lw)
plt.plot([x2, 1.0], [y1, y1], c="#111111", lw=lw)

# piet blocks
red_xs = [xs[i] for i in 1:n if xs[i] >= x1]
red_ys = [ys[i] for i in 1:n if xs[i] >= x1]
plt.fill_between(red_xs, y2, red_ys, fc="#FA0000")
white_xs = [xs[i] for i in 1:n if xs[i] <= x1]
white_ys = [ys[i] for i in 1:n if xs[i] <= x1]
plt.fill_between(white_xs, y2, white_ys, fc="#FFFFEE")
plt.fill_between([0, x1], 0, y2, fc="#0E75CF")
plt.fill_between([x2, 1], 0, y1, fc="#FFEC02")
plt.fill_between([x1, x2], 0, y2, fc="#FFFFEE")
plt.fill_between([x2, 1], y1, y2, fc="#FFFFEE")

# save
plt.savefig("./docs/src/assets/logo.svg", dpi=1000, transparent=true)
plt.close("all")
